...
local function bitchAboutIt(capRem, stop)
	local pct
-- set up interval
	if capRem < 10 then
		pct = capRem % 5
...
-- bitch about it.
	if pct == 0 and capRem ~= stop and capRem > 0 and capRem < 100 then
		playNumber(capRem, 13)
    stop = capRem	-- don't keep bitching about it.
  end
	return stop
end
---------------------------------------------------------------------------
...
local function background(zone)
  ...
  if getRSSI() > 0 then -- play percent if telemetry is linked up
    zone.widgetOptions.trip = bitchAboutIt(capRem,zone.widgetOptions.trip)
  end
end
